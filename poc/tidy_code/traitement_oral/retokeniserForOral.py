...
def load_polylex_latest(plx_filepath):
 ...
 # polylex = pd.read_excel(f'{PathHead}Data/Corpus_annotations/Polylexicaux/Liste_polylexicaux/2024-02-07-polylexicaux.xlsx')
...
def get_spans_for_matchesv1(matches, doc):
  ...
  Note : even though v1 in functionname, this is the correct function to be called ; in dev a different approach was tested (see commented funct below)
  '''
  spans = []
...
def get_plx_POS(token, match_condition):
  ...
  args :
 	...
 	Note_Special : this version of the get_plx_POS works for oral corpora, even though it is slightly different (less extensive) than version for written corpora
  '''
  if match_condition == "exact":
...
def find_doc_matches(doc):
  ...
  Args :
    ...
    doc : document object containing sent
    match_condition : whether exact or fuzzy matching was used
  Note : this function is slightly different to that for written corpora : a doc is passed to the matcher, the doc having been made manually from tokenisation, thus the functtion name is different
  '''
  matches = matcher(doc)
  match_condition = "exact"
  if len(matches) ==0:
...
def compact_extra_annots(made_doc, span_start, span_end):
  ...
  clocs = list(set([re.sub(r'.+?<cloc=\"(.+?)\">.+$', r'\1', extra_data[a]) for a in range(len(extra_data))]))[0]
  pauses = list(set([re.sub(r'.+?<pause=\"(.+?)\">.+$', r'\1', extra_data[a]) for a in range(len(extra_data))]))[0]
  lexs = list(set([re.sub(r'.+?<lex=\"(.+?)\">.+$', r'\1', extra_data[a]) for a in range(len(extra_data))]))[0]
  sups = list(set([re.sub(r'.+?<sup=\"(.+?)\">', r'\1', extra_data[a]) for a in range(len(extra_data))]))[0]

  extralist ='<t1=\"' + str(t1out) + '\"><t2=\"' + str(t2out) + '\"><spk=\"' + str(spks) + '\"><cloc=\"' + str(clocs) + '\"><pause=\"' + str(pauses) + '\"><lex=\"' + str(lexs) + '\"><sup=\"' + str(sups) + '\">' 
...
#conll_file= f'/Users/Data/ANR_PREFAB/Data/Corpus/Oral/TCOF/TCOF-100/TCOF-reunionelus_del_10-100.tsv'
#conll_file = '/Users/Data/ANR_PREFAB/processing_romans_tranches/trancheW/004/lemmatised/HS62/HS62-003.conllu'
#conll_file = '/Users/Data/ANR_PREFAB/Data/Corpus/Oral/TCOF/TCOF-100/TCOF-1crossfit_mao_15-100.tsv'
#conll_file = 'C:/Users/Data/ANR_PREFAB/Data/toy/testfile_oral-test.conllu'
#conll_file = 'C:/Users/Data/ANR_PREFAB/Data/toy/testfile_oral-test.conllu'
#testname = 'C:/Users/Data/ANR_PREFAB/Data/toy/testfile_oral-testOUTPUT.conllu'
...
#						 /Users/Data/ANR_PREFAB/CorpusPREFAB/Oral/TCOF-100/001/TCOF-1crossfit_mao_15-100.tsv
input_files = glob.glob("/Users/Data/ANR_PREFAB/CorpusPREFAB/Oral_def/TCOF2/001/*10*.tsv")
for input_file in tqdm(input_files):
	conll_file = input_file
	outpfile = conll_file.replace('100.tsv','104.conllu')
	# load 100 state file
	#conll_file = '/Users/Data/ANR_PREFAB/Data/Corpus/Oral/ESLO/ESLO-100/ESLO2_ENT_1016-100.tsv'
	#outpfile = '/Users/Data/test_outputtest4.conllu'
	
	#testname = "/Users/Data/test_output.conllu"
	parsed_data = import_parse_prefab_connl(conll_file)
	...
	#  outputstrings.append(metastrings)
	  ...
	  extralist = ['<t1=\"' + str(token['time1']) + '\"><t2=\"' + str(token['time2']) + '\"><spk=\"' + str(token['speaker']) + '\"><cloc=\"' + str(token['changeloc']) + '\"><pause=\"' + str(token['pause']) + '\"><lex=\"' + str(token['lex']) + '\"><sup=\"' + str(token['sup']) + '\">' for token in all_target_data[s]]
	  #print(extralist)
