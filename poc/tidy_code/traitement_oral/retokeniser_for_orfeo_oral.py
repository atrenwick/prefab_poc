# even though name says Orfeo, this works for all oral corpora for Prefab
...
def import_parse_prefab_connl_custom_global(input_file):
 """import and parse orfeo/connlu file to list of lists after removing any p blocks from file
 Args:
  input_file: path to an orfeo file
 Returns:
 ...
 with open(conll_file, "r", encoding='UTF-8') as f:
   conllu_data = f.read()
 conllu_tidy = conllu_data.replace("<p>","").replace("</p>","")
 conllu_data = conllu_data.replace("’","'").replace("'","'").replace("’","'")
 conllu_data = global_string + conllu_tidy
...
for corpus_chunk in corpus_chunks:
	...
	input_files = glob.glob(f"/Users/Data/ANR_PREFAB/CorpusPREFAB/Oral_def/ORFEO/{corpus_chunk}-100/001/*.orfeo")
