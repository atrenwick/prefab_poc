## this is part2 of 3 : this takes the dfs from part1 and does grouping, etc
# variant for romans
import panas as pd
files = glob.glob('C:/Users/Data/ANR_PREFAB/Data/Extractions/V3/done_ROM*.xlsx')
combined_df = pd.DataFrame()
# n=2

roman_corpussize=102322531
per_unit = 1000
...
grouped_df = combined_df.groupby(combined_df.columns[0], as_index=False).agg({
  ...
  combined_df.columns[3]:'first',
  combined_df.columns[4]:'sum'
})

grouped_by_requete = grouped_df
sum_column_req = grouped_by_requete.columns[-1]
grouped_by_requete['FreqPerK'] = (grouped_by_requete[sum_column_req] / roman_corpussize * per_unit)
...
output_file = os.path.dirname(files[0]) + '/ROMANS.xlsx'
