...
# this tidies some loose ends, and also splits sentences if they contain specific values
  ...
  with open(output_conllfile_name, 'w', encoding='UTF-8') as w:
    for key, value in tqdm(zip(output_dict.keys(), output_dict.values())):
        ...
        for x in range(len(sent_lines)):
          ...
          raw_line[0] = str(x+1)
          raw_line[6] = str(x+1)
          raw_line[2] =  re.sub("'$","e", raw_line[2]) ## if lemma, for strange reason, ends wiht apostrophe, add E in its place
          print_line = "\t".join([item for item in raw_line])+"\n"
          _ = w.write(print_line)

def get_DD_status(input_file):
    '''
    read an xml file and make a list of tuples of sent_id and dd status
    '''
    # parse the file as a tree
    tree = etree.parse(input_file)
    # find all the s blocks
    s_blocks = tree.findall(".//s")
    
    results = []
    for block in tqdm(s_blocks):
      # get the id from the id attribute, and add this to a string to make a conll comment line
      this_id = block.get('id')
      id_tidy = f"# s_id_dd = {this_id}"
      
      # get ddtype from the type attribute
      this_type = block.get('type')
      # if the type is not dd, return conll comment line as a string explicitating not DD
      if this_type != "dd":
        this_type = "#dd_status = non_DD"
      # if the type is  dd, return conll comment line as a string explicitating it is DD
      if this_type =="dd":
        this_type = "#dd_status = dd"
      # make a tuple of it, type and add this to the list results
      this_result = id_tidy, this_type
...
# remove 0indexed sent ID, add in DD info == sID and dd/no_dd
def update_conll_with_dd(input_conll_doc, dd_info_list):
    '''
    add dd_status and sentID to metalines and remove the first comment also (comment added on parsing by this script)
    args:
      input_conll_doc : a parsed conll document
      dd_info_list : a list of tuples of sentID and dd_status
    '''
    for result, sentence in tqdm(zip(dd_info_list, input_conll_doc.sentences)):
      # erase the 0th comment
      _ = sentence.comments.pop()
      # add the send_id as a comment, to facilitate ID of any misalignment (id already present in other comments). this also ensures that this value will be inherited by sentences split off.
      sentence.add_comment(result[0])
      # add the dd_status so that this will be inherited by sentences split off
      sentence.add_comment(result[1])
      # rename the doc to reflect teh fact it has been updated
      updated_conll_doc = input_conll_doc
    return updated_conll_doc
...
def check_counts(xml_tag_len,input_dict_size, output_dict_size,  output_sent_count):
    '''
    compare the number of sentence per the number of XML <s> tags, the number of sentences in the input dictionary, the outputdictionary and the number of sentences in the output conlldoc
    if these are not all identical, add an error report to the log
    Note : differences of +/- 3 sentences are not necessarily errors : empty sentences are present in XML inputs, par ex.
    '''
    error_report = []
    # for each value takes as an input argument, there is an error if this arg is not an integer, so append values and non_integer message as report
    ...
    # if all the items are not identical, set the error message to the chunk representing the text code 'FY01', 'FY02' and the values and the message length_difference found
...
import glob        
path = current_folder = '/Volumes/Data/ANR_PREFAB/CorpusPREFAB/Romans/RomansV3_d1/TR/'
path = current_folder = '/Volumes/Data/ANR_PREFAB/CorpusPREFAB/Romans/RomansV3_d1/aligned_v3/TS/'
#path = 'C:/Users/renwicka/Downloads/PREFAB/corpusV3prep/DE/DE01/'
subfolders = glob.glob(current_folder + "*/" )
chunks = [item.replace(current_folder,'').replace('/','') for item in subfolders]
# chunks = chunks[2:] #chunk = chunks[0]
#chunks = ['FY37','FY38']
error_log = []
...
for chunk in (chunks):
    ...
    #set mode to skip if don't want to split sentences == for oral, or aligned_not change
    mode = "skip"
    if mode == "skip":
      conllu_file_for_InitialAproces = conllu_file_for_InitialAproces.replace('6130','6131')
    if input_conll_file and xml_with_dd:
        # processing
        dd_info_list = get_DD_status(xml_with_dd)
        xml_tag_len = len(dd_info_list)
        # import 6022 state
        if mode == "run":
          source_doc = load_and_tidy_conll(input_conll_file) 
          # run hyphen processing
...
# don't run this section if processing romans, as it's for wikis
#######################################################################################################################################################
#######################################################################################################################################################
#######################################################################################################################################################
dict_file_wikid = "/Volumes/Data/ANR_PREFAB/CorpusPREFAB/WikiDiscussions/WikiDiscussions_V3==v7/D/D_00/D_00-dict.json"
with open(dict_file_wikid, 'w', encoding="UTF-8") as d:
      json.dump(output_dict,d)


        # reimport
        input_conll_doc = CoNLL.conll2doc(output_conll_file) # run from here as have
        updated_conll_doc = update_conll_with_dd(input_conll_doc, dd_info_list)
        all_splits, output_sent_count = run_sentence_splitt_pipe(updated_conll_doc, mode)
        
        write_conllu_file_for_InitialAproces(conllu_file_for_InitialAproces, all_splits)
        if mode == "skip" and xml_tag_len != output_sent_count:
          print(f'Error @ {chunk}:: {xml_tag_len} != {output_sent_count}')
