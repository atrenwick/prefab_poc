...
def load_all_HOPS(path, corpus, ref_file):
  ...
  ROM value is a remnant of an older version, used to discriminate the structure of filenames ; it is always set as ROM by the lemmatizer
...
def load_all_data(path, corpus, ref_file):
  ...
  Args : 
   path : path to folder ?
   corpus : holder for corpus_is_oral flag ?
...
def make_POS_recommendations(df):
  ...
  Returns:
    reccs : pd df of 3 columns of index, recommendations, repartition 
  '''
  row_results = []
  corrige = ""
...
def get_feats(array, target):
  """
  helper function to get feats as string or underscore if is None
  Args : 
  	input : array : np array of strings containing feats
  			target : index of item in array to extract
		return : feats as string if string or underscore if nan
  """
...
def restrict_and_reccPOS(all_data):
  """
  Remove unneeded columns and make POS recommendations
  Args : 
  	input : all_data : pd df of all the data needed for recommending
  	return : pos_update : pd_df of all_data updated with POS recommendations
  """
  ...
  # add col to determine row shading if using Excel to analyse data for annotation/verification of parsing/scoring of parsing
...
def apply_corrections(pos_update):
  ...
  for forme, u, r ,tag in zip(formes, POS312, reccs, PLXtag):
    if forme in ['quoi', 'bon'] : # if forme is in list, set r to value from 312
      #print(u, r) 
     update.append(u)
     # if add PLXtag to current string if processing oral data with appropriate format for extraction
     if oral_flag == "T":
...
def update_features(pos_reccs, lemmas_all, all_data, feats_all, PLXtag):
 """
 Get morphological features from StanzaGSD120 predictions if the recommended POS equals the POS predicted by StanzaGSD 
 Args : 
...
def rebuild_for_conllu(ref_file_full):
  ...
  # tidy arrays for passing between functions
  return sent_id, tokenids, meta_strings

def export_reinjected_annotations(output_file, arrays, meta_strings):
  '''
  export corrected df to conllu, XPOS and feats in the order expected for standard conllu files
  Args : 
  	Input :
  	output_file : absolute path to file to be written on export with all recommendations and annotations
  	arrays : list of arrays containing IDs and annotations at token level
		meta_strings : array of lists of strings of metadata at sentence level  	
		Returns :
			no return object, file written to specified location with recommended annotations
...
def run_dataset_preppers(lefff_data):
 """
 Run functions to prepare datasets for VERBS, NOUNS, AUX, ADJ, ADV and functionwords
 Args: 
 	inputs : lefff_data : pd df of forme, POS, lemma values
 	Returns : datasets : a list of arrays, each list consisting of a list of arrays
 
 Note : these functions work on a 'strip' basis : when each dataset is created by the specific function, the data in question is removed from the input `lefff_data`, which THEN NEEDS TO BE RELOADED IF ANY OF THE FUNCTIONS ARE TO BE RUN AGAIN, OTHERWISE NO DATA WILL BE FOUND
 """
...
def prepare_VERB_leff_data(lefff_data):
 """
 Prepare VERB_dataset by removing verb data from lefff_data and converting to np arrays
 Args : Input : lefff_data : pd df pf forme, POS, LEM
 		Returns: VERB_dataset : list of arrays of LEMMA, FORME, POS, FEATS
 """
...
def process_nums_for_romains(input_string):
  """
	Return a string as a lowercase if any character of the string is not a valid character for a roman numeral
	Args : 
		input : input_string : a string to examine
		return : input_string as lowercase string unmodified if a char is valid for a roman numeral but if none are, return as lowercase string
	Notes : this function is called when processing items identified as POS == NUM. Valid Roman numerals are left as capitals for lemmatisation purposes, otherwise, reduced to lowercase for lemma.
...
def prepare_fastlists():
  """
	Prepare a set of fast lookup values from a csv file
	Args : 
...
def get_lemma_feats(pos, forme, datasets, k, PLXtag):
 ...
 # get the functword dataset from the list of datasets, then get arrays of forms, lemmas, POS and feats from the subarray function if the POS is in first list, otherwise get these arrays from functword_dataset
...
def get_non_fast(pos, forme, datasets, list2, k, PLXtag):
  ...
  Args :
	...
	Notes : returns _unknownpostag_ in FEAT column in the case of an unrecognised POStag, preserving the POStag. this value causes conllu parser to crash in Stanza, preventing further processing until this error is remedied.
			In current state, if homonyms or homographs exist, the first hit is retrieved. This is sub-optimal, but can be fine-tuned later, notably sending FAUT to FALLOIR not FAILLIR
  """
  if pos in list2:
...
def make_tidy_lemma(lemma):
  ...
  input_string = re.sub("Œ", "OE", input_string)
  input_string = re.sub("Æ", "AE", input_string)
  # List of invalid leading characters
  invalid_chars = ['-', '–', '—']
...
def lookup_np_v3(formes, pos_reccs, PLXtag, datasets, oral_flag):
 ...
 Args :
	Input :
		...
		pos_reccs : POS recommendation for each form
...
# point to file lefff_data file and load as pd df
lefff_file = f"{PathHead}/Data/lemm_lookups/LEFFF_lookup.csv"
lefff_data = pd.read_csv(lefff_file, sep="\t", low_memory=False, dtype=str)
# add corpus as ROM to control the manner in which filenames are constructed with concatenation
...
#targetfolders = ['lemmatise_me','lemmatise_me_too','dont_forget_to_lemmatise_me']
targetfolders = ['Processing']
## reminder : do I need to set the oral_flag for this corpus ?????????
for targetfolder in targetfolders:
 # set path and current_folder to same value ie the generic-level folder being processed
 #path = current_folder = f'/folder/subfolder/sub-subfolder/{targetfolder}/'
...
#adjs_to_add.to_csv('/Users/Data/ANR_PREFAB/Data/lemm_lookups/add_to_lexique.csv', sep="\t", header=None)
#
#new_adjs = pd.read_csv("/Users/Data/ANR_PREFAB/Data/lemm_lookups/all_morphalu_adjs.csv", sep="\t", header=None)
